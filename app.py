...
def make_orbital_cube(mol, conf_id, orbital_type="HOMO"):
    ...
    min_bounds = pos.min(axis=0) - 3
    max_bounds = pos.max(axis=0) + 3
    grid_res = 20
    ...
    seed = 1 if orbital_type == "HOMO" else 2
    np.random.seed(seed)
    ...
    for i in range(min(n_atoms, 8)):
        ...
        grid_data += phase * np.exp(-dist_sq / 1.5)
        
    cube_header = f"Orbital Cube\nGenerated by Gemini\n{n_atoms} {min_bounds[0]} {min_bounds[1]} {min_bounds[2]}\n"
    cube_header += f"{grid_res} {(max_bounds[0]-min_bounds[0])/grid_res} 0 0\n"
    cube_header += f"{grid_res} 0 {(max_bounds[1]-min_bounds[1])/grid_res} 0\n"
    cube_header += f"{grid_res} 0 0 {(max_bounds[2]-min_bounds[2])/grid_res}\n"
    ...
    for i in range(0, len(flat_data), 6):
        cube_body += " ".join(map(lambda val: f"{val:12.6E}", flat_data[i:i+6])) + "\n"
...
if mol_raw:
    ...
    sel_id = st.selectbox("Select Conformer", sorted_ids)
    ...
    with v1:
        ...
        if "Lobes" in view_mode:
            orb_type = "HOMO" if "HOMO" in view_mode else "LUMO"
            cube_data = make_orbital_cube(mol_hs, sel_id, orb_type)
            view.addVolumetricData(cube_data, "cube", {'isoval': 0.15, 'color': "blue", 'opacity': 0.85})
            view.addVolumetricData(cube_data, "cube", {'isoval': -0.15, 'color': "red", 'opacity': 0.85})
        else:
            view.addSurface(py3Dmol.VDW, {'opacity': 0.2, 'color': 'white'})
            
        view.zoomTo()
        showmol(view, height=450, width=600)
        st.caption("🔴 Negative Phase | 🔵 Positive Phase")

    with v2:
        st.write("**Orbital Energy**")
        fig_gap = go.Figure()
        ...
        fig_gap.update_layout(yaxis_title="Energy (eV)", height=400, showlegend=False)
        st.plotly_chart(fig_gap, use_container_width=True)

    st.markdown("### 3. Coordinate Mapping")
    st.dataframe(get_internal_coordinates(mol_hs, sel_id), use_container_width=True)
else:
